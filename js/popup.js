    ...
    `}getStatusCard(){return`
      <div class="status-card">
        ...
        <div style="margin-top:10px; font-size:12px; color:var(--text-muted); display:flex; align-items:center; gap:8px;">
          <span>\u{1F500} Navigation URL rewrites: ${this.state.dnrActiveRules||0} active</span>
          <button class="btn btn-secondary btn-xs btn-resync-dnr" title="Resync navigation redirects now">\u21BB</button>
        </div>
        ${this.state.dnrDiag&&this.state.dnrDiag.error?`<div style="margin-top:4px; font-size:11px; color:var(--danger, #dc3545);">\u26A0 ${this.escapeHtml(this.state.dnrDiag.error)}</div>`:""}
      </div>
