...
`).map(s=>s.trim()).filter(s=>s.length>0):Array.isArray(e)&&(t=e.map(s=>String(s).trim()).filter(s=>s.length>0)),t.length===0?"":t.length===1?t[0]:t}addFindReplacePair(){try{this.newRule.findReplacePairs||(this.newRule.findReplacePairs=[]),this.newRule.findReplacePairs.push({find:"",replace:"",useRegex:!1,targetType:"all"}),this.updateModalPairs()}catch(e){console.error("Error adding find/replace pair:",e)}}removeFindReplacePair(e){try{if(!this.newRule.findReplacePairs||this.newRule.findReplacePairs.length<=1)return;e>=0&&e<this.newRule.findReplacePairs.length&&(this.newRule.findReplacePairs.splice(e,1),this.updateModalPairs())}catch(t){console.error("Error removing find/replace pair:",t)}}_getBlockOptions(){return[["findReplace","Find & Replace"],["setJson","Set JSON Field"],["deleteJson","Delete JSON Field"],["renameJson","Rename JSON Field"],["setHeader","Set Header"],["removeHeader","Remove Header"],["renameHeader","Rename Header"],["setUrlParam","Set URL Param"],["removeUrlParam","Remove URL Param"],["capture","Capture from Response"]]}_getValueKindOptions(){return[["text","Text"],["random","Random Number"],["timestamp","Timestamp"],["capture","Captured Value"]]}_newBlock(e){let t={type:e||"findReplace",valueKind:"text",params:{}};switch(t.type){case"findReplace":t.params={find:"",value:"",targetType:"all",urlMatch:"",useRegex:!1};break;case"setJson":t.params={path:"",valueKind:"text",value:"",min:"",max:"",name:"",unit:"s"};break;case"deleteJson":t.params={path:""};break;case"renameJson":t.params={path:"",newName:""};break;case"setHeader":t.params={header:"",valueKind:"text",value:"",min:"",max:"",name:"",unit:"s",scope:"request"};break;case"removeHeader":t.params={header:""};break;case"renameHeader":t.params={header:"",newName:""};break;case"setUrlParam":t.params={paramName:"",valueKind:"text",value:"",min:"",max:"",name:"",unit:"s"};break;case"removeUrlParam":t.params={paramName:""};break;case"capture":t.params={name:"",pattern:""};break}return t}addBlock(){this.newRule.blocks||(this.newRule.blocks=[]),this.newRule.blocks.push(this._newBlock("findReplace")),this.updateBlocksList()}removeBlock(e){this.newRule.blocks&&e>=0&&e<this.newRule.blocks.length&&(this.newRule.blocks.splice(e,1),this.updateBlocksList())}moveBlock(e,t){if(!this.newRule.blocks)return;let s=e+t;if(s<0||s>=this.newRule.blocks.length)return;let a=this.newRule.blocks[e];this.newRule.blocks[e]=this.newRule.blocks[s],this.newRule.blocks[s]=a,this.updateBlocksList()}updateBlockType(e){if(!this.newRule.blocks||!this.newRule.blocks[e])return;let t=this.newRule.blocks[e],s=document.querySelector(`select.blockType[data-block-idx="${e}"]`),a=s?s.value:t.type;if(a===t.type)return;let l=this._newBlock(a);l.params=Object.assign({},l.params,{path:t.params.path||l.params.path,header:t.params.header||l.params.header,name:t.params.name||l.params.name,paramName:t.params.paramName||l.params.paramName,find:t.params.find||l.params.find}),this.newRule.blocks[e]=l,this.updateBlocksList()}updateBlockValueKind(e){if(!this.newRule.blocks||!this.newRule.blocks[e])return;let t=document.querySelector(`select.block-value-kind[data-block-idx="${e}"]`);t&&(this.newRule.blocks[e].valueKind=t.value||"text",this.newRule.blocks[e].params||(this.newRule.blocks[e].params={})),this.updateBlocksList()}_compileBlockValue(e,t,s){if(t||(t={}),e==="random"){let a=parseInt(t.min,10),l=parseInt(t.max,10);(isNaN(a)||isNaN(l)||a>l)&&(a=0,l=999999);let i=(t.name||"").trim();if(!i){let n=0;do i="v"+n,n++;while(s.has(i));s.add(i)}return`$var{${i},${a},${l}}`}if(e==="timestamp")return t.unit==="ms"?"$ts{ms}":"$ts{}";if(e==="capture"){let a=(t.name||"").trim();return a?`$capture{${a}}`:""}return t.value!==void 0&&t.value!==null?String(t.value):""}_compileBlocks(e,t){let s={pairs:[],jsonFields:[],headerRules:[],urlParams:[],captures:[]};if(!Array.isArray(e))return s;let a=new Set,l=t==="block"||t==="mock"||t==="delay"||t==="inject";for(let i of e){if(!i||!i.type)continue;let n=i.params||{},c=i.valueKind||"text";switch(i.type){case"findReplace":{let o=(n.find||"").trim();if(!o)break;let u=this._compileBlockValue(c,n,a);s.pairs.push({find:o,replace:u,useRegex:!!n.useRegex,targetType:n.targetType||"all",urlMatch:(n.urlMatch||"").trim()});break}case"setJson":{let o=(n.path||"").trim();if(!o)break;let u=this._compileBlockValue(c,n,a);s.jsonFields.push({action:"set",path:o,value:u,newName:""});break}case"deleteJson":{let o=(n.path||"").trim();o&&s.jsonFields.push({action:"delete",path:o,value:"",newName:""});break}case"renameJson":{let o=(n.path||"").trim(),u=(n.newName||"").trim();o&&u&&s.jsonFields.push({action:"rename",path:o,value:"",newName:u});break}case"setHeader":{let o=(n.header||"").trim();if(!o)break;let u=this._compileBlockValue(c,n,a);s.headerRules.push({action:"set",header:o,value:u,newName:"",scope:n.scope==="response"?"response":"request"});break}case"removeHeader":{let o=(n.header||"").trim();o&&s.headerRules.push({action:"remove",header:o,value:"",newName:"",scope:"request"});break}case"renameHeader":{let o=(n.header||"").trim(),u=(n.newName||"").trim();o&&u&&s.headerRules.push({action:"rename",header:o,value:"",newName:u,scope:"request"});break}case"setUrlParam":{let o=(n.paramName||"").trim();if(!o)break;let u=this._compileBlockValue(c,n,a);s.urlParams.push({action:"set",name:o,value:u});break}case"removeUrlParam":{let o=(n.paramName||"").trim();o&&s.urlParams.push({action:"delete",name:o,value:""});break}case"capture":{let o=(n.name||"").trim(),u=(n.pattern||"").trim();o&&u&&s.captures.push({name:o,pattern:u});break}default:break}}return l?{pairs:[],jsonFields:s.jsonFields,headerRules:s.headerRules,urlParams:[],captures:s.captures}:s}addJsonField(){this.newRule.jsonFields||(this.newRule.jsonFields=[]),this.newRule.jsonFields.push({action:"set",path:"",value:"",newName:""}),this.updateJsonFieldsList()}removeJsonField(e){this.newRule.jsonFields&&e>=0&&e<this.newRule.jsonFields.length&&(this.newRule.jsonFields.splice(e,1),this.updateJsonFieldsList())}addHeaderRule(){this.newRule.headerRules||(this.newRule.headerRules=[]),this.newRule.headerRules.push({action:"set",header:"",value:"",newName:"",scope:"request"}),this.updateHeaderRulesList()}removeHeaderRule(e){this.newRule.headerRules&&e>=0&&e<this.newRule.headerRules.length&&(this.newRule.headerRules.splice(e,1),this.updateHeaderRulesList())}addUrlParam(){this.newRule.urlParams||(this.newRule.urlParams=[]),this.newRule.urlParams.push({action:"set",name:"",value:""}),this.updateUrlParamsList()}removeUrlParam(e){this.newRule.urlParams&&e>=0&&e<this.newRule.urlParams.length&&(this.newRule.urlParams.splice(e,1),this.updateUrlParamsList())}_urlParamRowsHtml(){return(this.newRule.urlParams||[]).map((t,s)=>`
		...
		`).join("")}updateUrlParamsList(){let e=document.querySelector(".url-params-list");if(!e)return;(this.newRule.urlParams||[]).length===0?e.innerHTML='<div class="empty-hint">No URL parameter operations. Click "+ Add Param" to add one.</div>':e.innerHTML=this._urlParamRowsHtml()}_paramOpsToPairs(e){let t=[];for(let s of e||[]){let a=(s.name||"").trim();if(!a)continue;let l=a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");s.action==="delete"?(t.push({find:"([?&]"+l+"=[^&]*)(?=&)",replace:"$1",useRegex:!0,targetType:"url",_fromParam:!0}),t.push({find:"[?&]"+l+"=[^&]*$",replace:"",useRegex:!0,targetType:"url",_fromParam:!0}),t.push({find:"&&",replace:"&",useRegex:!1,targetType:"url",_fromParam:!0})):t.push({find:"([?&]"+l+"=)[^&]*",replace:"$1"+(s.value||"").replace(/\$/g,"$$$$"),useRegex:!0,targetType:"url",_fromParam:!0})}return t}_blockValueEditor(e,t,s){let a=t.params||{},l=t.valueKind||"text",i=this._getValueKindOptions().map(([c,o])=>`<option value="${c}" ${l===c?"selected":""}>${o}</option>`).join(""),n="";if(l==="random")n=`
			...
			<div class="block-value-editor" style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
				...
				${n}
			</div>`}_blockRowHtml(e,t){let s=e.params||{},a=this._getBlockOptions().map(([o,u])=>`<option value="${o}" ${e.type===o?"selected":""}>${u}</option>`).join(""),l=this._getBlockOptions().find(([o])=>o===e.type),i=l?l[1]:"Block",n=`block-${t}`,c="";switch(e.type){case"findReplace":c=`
			...
			`).join("")}updateModalPairs(){try{let e=document.querySelector(".find-replace-pairs-list");if(!e){console.warn("Pairs list element not found");return}if(!this.newRule.findReplacePairs||!Array.isArray(this.newRule.findReplacePairs)){console.warn("Invalid findReplacePairs data");return}let t=this.newRule.findReplacePairs.map((s,a)=>{let l={find:s?.find||"",replace:s?.replace||"",useRegex:!!s?.useRegex,targetType:s?.targetType||"all",urlMatch:s?.urlMatch||""};return`
					<div class="find-replace-pair">
						...
						<label>Replace</label>
						<textarea name="pair-${a}-replace" class="form-input" rows="2" placeholder="Supports $rand{}, $var{}, $ts{}, $capture{}">${this.escapeHtml(l.replace)}</textarea>
						<label>Target</label>
						<select name="pair-${a}-targetType" class="form-input form-input-sm">${this._pairTargetOptions(l.targetType)}</select>
						<label>URL Match (optional \u2014 overrides the rule-level match for this pair)</label>
						<input type="text" name="pair-${a}-urlMatch" class="form-input form-input-sm" placeholder="e.g. api.example.com/*" value="${this.escapeHtml(l.urlMatch||"")}">
    ...
    `}getDefaultRulesPanel(e){let t=this.state.categories||[],s=this.state.activeCategory||"all",a=`<button class="category-tab ${s==="all"?"active":""}" data-category="all" style="--tab-color:#888">All (${e.length})</button>`;for(let i of t){let n=e.filter(c=>c.category===i.id).length;n>0&&(a+=`<button class="category-tab ${s===i.id?"active":""}" data-category="${this.escapeHtml(i.id)}" style="--tab-color:${this.escapeHtml(i.color||"#888")}">${this.escapeHtml(i.name)} (${n})</button>`)}let l=this._filterRules(s==="all"?e:e.filter(i=>i.category===s));return`
        ...
        <div class="rules-panel">
          ...
          <div class="panel-content">
            ...
            <div class="rules-list-panel">
              ${l.map(i=>this.getCompactRuleItem(i)).join("")}
            </div>
            `:`
    ...
    `}let t=this._ruleTypeBadges(e);e.findReplacePairs&&e.findReplacePairs.length>0&&e.findReplacePairs.some(n=>n.useRegex)&&t.push("REGEX");let a=(this.state.ruleStats||{})[e.title||"Untitled Rule"];a&&t.push("\u26A1"+a.hits),e.urlMatch&&(Array.isArray(e.urlMatch)?e.urlMatch.length>0:e.urlMatch.trim()!=="")&&t.push("MATCH");let i=t.join(" \u2022 ");return`
      <div class="rule-item-compact ${e.enabled?"enabled":"disabled"}">
        <div class="rule-compact-header">
          <div class="rule-toggle-small ${e.enabled?"active":""}" 
               data-rule-id="${e.id}" 
        ...
        <div class="rule-header">
          ...
          <div class="rule-info">
            <div class="rule-title-row">
              <span class="rule-title">${l?"\u{1F512} ":""}${this.escapeHtml(t)}</span>
            ...
            <div class="rule-meta">
              ...
              <span class="rule-type-badge">${l?"\u{1F512} PROTECTED":f}</span>
              ${l?'<span class="rule-desc">Rule body is encrypted \u2014 cannot be viewed or exported</span>':""}
